...
namespace QL_BanHang.Model
{
    class HoaDonMod
    {
        ...
        public DataTable GetDataByKH(string MaKH)
        {
            DataTable dt = new DataTable();
            cmd.CommandText = @"select hd.MaHD, hd.NgayLap, nv.TenNV, kh.TenKH from tb_HoaDon hd, tb_KhachHang kh, tb_NhanVien nv where kh.MaKH = hd.KhachHang and nv.MaNV = hd.NguoiLap and hd.KhachHang = '" + MaKH + "'";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con.Connection;
            try
            {
        ...
        public bool DelDataKhachHang(string maKH)
        {
            cmd.CommandText = "Delete tb_HoaDon Where KhachHang = '" + maKH + "'";
            cmd.CommandType = CommandType.Text;
            cmd.Connection = con.Connection;
            try
            {
